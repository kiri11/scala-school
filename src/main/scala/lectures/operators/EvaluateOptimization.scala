...
object EvaluateOptimization extends App with Data {
  ...
  val partiallyAppliedCurriedFunction = CurriedComputation.curriedComputation(filterData) _
  for (i <- 1 to 100) {
  ...
  val filterApplied = FunctionalComputation.functionalComputation(filterData)
  // ВЫПОЛНИТЬ В ЦИКЛЕ ОТ 1 ДО 100 FunctionalComputation.filterApplied
  for (i <- 1 to 100) {
    filterApplied(dataArray)
  }
